...
| `scylla.worker.config.max.bytes` | Maximum UTF-8 size of the internal stream assignments placed in one Kafka Connect task configuration. The default is `786432` (768 KiB), leaving headroom below Kafka's default 1 MiB request and record limits. Assignments use conservative byte-aware packing; if this limit is reached, increase `tasks.max` or split tables across connectors. Raise this setting only after increasing the corresponding Kafka limits. |
