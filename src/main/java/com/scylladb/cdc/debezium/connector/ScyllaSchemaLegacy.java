...
/**
 * Legacy schema implementation that wraps non-PK column values in a Cell struct.
 *
 ...
 */
public class ScyllaSchemaLegacy implements DatabaseSchema<CollectionId> {
  private static final Logger LOGGER = LoggerFactory.getLogger(ScyllaSchemaLegacy.class);
  ...
  public ScyllaSchemaLegacy(ScyllaConnectorConfig configuration, Schema sourceSchema) {
  ...
  private Map<String, Schema> computeCellSchemas(
    ...
    for (ChangeSchema.ColumnDefinition cdef : changeSchema.getNonCdcColumnDefinitions()) {
      ...
      if (!isSupportedColumnSchema(cdef)) {
        LOGGER.debug(
            "Skipping unsupported column type {} for column {} in table {}.{}",
  ...
  private Schema computeAfterSchema(
    ...
    return computeValueSchema(changeSchema, cellSchemas, collectionId, ".After");
  }

  private Schema computeBeforeSchema(
      ChangeSchema changeSchema, Map<String, Schema> cellSchemas, CollectionId collectionId) {
    return computeValueSchema(changeSchema, cellSchemas, collectionId, ".Before");
  }

  private Schema computeValueSchema(
