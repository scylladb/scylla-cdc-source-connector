    ...
    public static int validateClusterIpAddresses(Configuration config, Field field, Field.ValidationOutput problems) {
        ...
        for (String clusterIpAddress : splitClusterIpAddresses) {
            ...
            if (hostPort.length != 2) {
                problems.accept(field, clusterIpAddress, "Expected host specification as a comma-separated list of pairs <IP>:<PORT>, but got invalid: " + clusterIpAddress);
                count++;
                continue;
            }
            ...
            if (!StringUtils.isNumeric(port) || port.length() > 5) {
                ...
                problems.accept(field, clusterIpAddress, "Expected host specification as a comma-separated list of pairs <IP>:<PORT>, but got invalid port number: " + clusterIpAddress);
                count++;
                continue;
            }
            ...
            try {
                ...
                problems.accept(field, clusterIpAddress, "Expected host specification as a comma-separated list of pairs <IP>:<PORT>, but got invalid <IP>:<PORT> pair: " + clusterIpAddress);
                count++;
            }
        }
    ...
    public static int validateTableNames(Configuration config, Field field, Field.ValidationOutput problems) {
        ...
        for (String tableName : splitTableNames) {
            ...
            if (keyspaceAndName.length != 2) {
                problems.accept(field, tableName, "Expected a comma-separated list of pairs <keyspace name>.<table name>, but got invalid: " + tableName);
                count++;
                continue;
            }
