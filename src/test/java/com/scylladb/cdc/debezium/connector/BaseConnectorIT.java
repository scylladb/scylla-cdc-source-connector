...
public abstract class BaseConnectorIT {
    ...
    protected static final KafkaContainer kafkaContainer = new KafkaContainer(DockerImageName.parse("confluentinc/cp-kafka:7.3.12"))
            .withNetwork(network);

    protected static final ScyllaDBContainer scyllaDBContainer = new ScyllaDBContainer("scylladb/scylla:6.2")
            .withNetwork(network)
    ...
    static {
        Startables.deepStart(
            Stream.of(kafkaContainer, scyllaDBContainer, debeziumContainer)
        ).join();
    }
